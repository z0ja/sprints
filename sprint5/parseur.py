...
def extract_title_from_txt(text):
    ...
    for line in lines[:15]:
        ...
        if title_lines and _looks_like_name(line):
            break
        title_lines.append(line)
...
def build_xml(article, txt_content, emails):
    ...
    SubElement(root, "titre").text = title or "N/A"
    ...
    if not authors_list:
        ...
        SubElement(auteur_el, "name").text = "N/A"
        SubElement(auteur_el, "mail").text = "N/A"
        SubElement(auteur_el, "affiliation").text = "N/A"
    else:
        for author_name in authors_list:
            ...
            SubElement(auteur_el, "mail").text = email or "N/A"
            affil = find_affiliation(author_name, email)
            SubElement(auteur_el, "affiliation").text = affil or "N/A"

    SubElement(root, "abstract").text = extract_abstract(txt_content) or "N/A"
    ...
    SubElement(root, "biblio").text = extract_references(txt_content) or "N/A"
...
def main():
    ...
    if args.all:
        selected_fnames = sorted([f for f in os.listdir(pdf_dir) if f.lower().endswith(".pdf")])
    else:
...
def parse_args():
    ...
    parser.add_argument("--all", action="store_true", help="Traiter tous les PDF sans demander")
